...
import sys
sys.path.insert(1, 'C:/Users/carla/IFSS-Net/')
...
x = tf.keras.Input(shape=(time_step, H, W, C), name="x", dtype=tf.float32)
y = tf.keras.Input(shape=(time_step, H, W, num_classes), name="y", dtype=tf.float32)

x_target = tf.keras.Input(shape=(time_step, H, W, C), name="x_target", dtype=tf.float32)
x_ref = tf.keras.Input(shape=(time_step, H, W, C), name="x_ref", dtype=tf.float32)

y_reference = tf.keras.Input(shape=(time_step, H, W, num_classes), name="y_reference", dtype=tf.float32)
y_estimated = tf.keras.Input(shape=(time_step, H, W, num_classes), name="y_estimated", dtype=tf.float32)

# define the placeholder for dropout
keep_prob = tf.keras.Input(shape=(), dtype=tf.float32, name="dropout_probability")
keep_prob_input = tf.keras.Input(shape=(), dtype=tf.float32, name="dropout_probability_input")
keep_prob_skip = tf.keras.Input(shape=(), dtype=tf.float32, name="SkipDropout")
...
keep_prob_alpha = tf.keras.Input(shape=(), dtype=tf.float32, name="drop_alpha")
keep_prob_beta = tf.keras.Input(shape=(), dtype=tf.float32, name="drop_beta")
keep_prob_tp = tf.keras.Input(shape=(), dtype=tf.float32, name="drop_tp")

# In[26]:

# defined as variables ?
Batch_size = x.shape[0]#tf.shape(x)[0]
time_size = x.shape[1]#tf.shape(x)[1]
...
P_foreground_SOL = Dropout(rate=1 - (keep_prob_tp),name="P_foreground_drop_out_SOL")(P_foreground_SOL)
...
def sum_reductions(tensor, axes):
    return tf.reduce_sum(tf.reduce_sum(tensor, axis=axes[0]), axis=axes[1])

true_positive_sum = layers.Lambda(lambda x: sum_reductions(x, [2, 3]))(true_positive_SOL)
false_pos_sum = layers.Lambda(lambda x: tf.reduce_sum(W_tanh[..., 0] * sum_reductions(x, [2, 3])))(false_pos_SOL)
false_neg_sum = layers.Lambda(lambda x: tf.reduce_sum(W_tanh[..., 1] * sum_reductions(x, [2, 3])))(false_neg_SOL)
...
TverskyIndex_SOL = layers.Lambda(lambda x: (x[0] + smooth) / (x[0] + x[1] + x[2] + smooth))([true_positive_SOL, false_pos_SOL, false_neg_SOL])
...
dice_loss_SOL = layers.Lambda(lambda x: 1 - tf.reduce_mean(x))(TverskyIndex_SOL)
...
train_step = optimizer.minimize(dice_loss_SOL, global_step=global_step)

with tf.compat.v1.name_scope('Optimizer'):
    train_step = tf.compat.v1.train.AdamOptimizer(learning_rate=learning_rate).minimize(dice_loss_SOL,global_step=global_step)
...
with tf.compat.v1.name_scope("Precision_Recall"):
...
model_path = 'C:/Users/carla/Documents/Master_Thesis/IFSS_data/model'
...
init_o = tf.compat.v1.global_variables_initializer()
Modelsummary={}
isRestor=False
...
x_y_path = 'C:/Users/carla/Documents/Master_Thesis/IFSS_data/train/'
...
#TrainSetDataPath = list(map(lambda s : s+'x1.mha' , #Patient_folder_full_path))
TrainSetDataPath = list(map(lambda s : s + 'data/', Patient_folder_full_path))
#TrainSetMaskPath = list(map(lambda s : s+'masksX1.mha' , Patient_folder_full_path))
TrainSetMaskPath = list(map(lambda s : s+'masks/' , Patient_folder_full_path))

#TestSet

x_y_path = 'C:/Users/carla/Documents/Master_Thesis/IFSS_data/test/'
Patient_folder_path = sorted(os.listdir(x_y_path))
Patient_folder_full_path = list(map(lambda v : str(join(x_y_path,v)) + '/', Patient_folder_path))

# Get the full path for the volume and the mask
DataTestPath = list(map(lambda s : s+'data/' , Patient_folder_full_path))
MasksTestPath = list(map(lambda s : s+'masks/' , Patient_folder_full_path))
...
# Define the path 
x_y_path = 'C:/Users/carla/Documents/Master_Thesis/IFSS_data/val/'
# define the full path for each patient
Patient_folder_path = sorted(os.listdir(x_y_path))
Patient_folder_full_path = list(map(lambda v : str(join(x_y_path,v)) + '/', Patient_folder_path))

# Get the full path for the volume and the mask
DataValPath = list(map(lambda s : s+'data/' , Patient_folder_full_path))
MasksValPath = list(map(lambda s : s+'masks/' , Patient_folder_full_path))
...
with tf.compat.v1.Session() as sess:
    ...
    for iterations in range(10):
        ...
        for subj in range(len(TrainSetDataPath)):
            ...
            image_dataset = []
            for path in glob.glob(TrainSetDataPath[subj]):
            ...
            mask_dataset = []
            for path in glob.glob(TrainSetMaskPath[subj]):
                for mask_path in glob.glob(os.path.join(path, "*.jpg")):
            ...
            #SequenceLength= lengthSeq
            SequenceLength = 5
             #randomly sampled over each iteration to make the network robust

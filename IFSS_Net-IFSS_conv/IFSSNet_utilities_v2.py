...
def AtrousSpatialPyramidPoolingModule_3D(inputs, depth=256):
    ...
    #     from original code: image_features = tf.transpose(tf.stack(list(map(lambda a:
    ...
    atrous_pool_block_1 = Conv3D(depth, (1, 1, 1), activation=None)(inputs)
...
def unpool(layer):
    ...
    # return tf.image.resize(layer, [2*int(layer.get_shape()[1]), 2*int(layer.get_shape()[2])], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
...
def weight_variable(shape):
    ...
    # weights_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0,mode="fan_avg",distribution=("uniform" if True else "truncated_normal"), seed=None, dtype=tf.float32)
...
def weight_variable2(shape):
    ...
    # weights_initializer=tf.compat.v1.random_uniform_initializer(minval=0.5, maxval=0.5, seed=None)

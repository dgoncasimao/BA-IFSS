...
# In[ ]:
save_path = 'C:/Users/carla/Documents/Master_Thesis/IFSS_data/analysis'
model_path = 'C:/Users/carla/Documents/Master_Thesis/IFSS_data/model/'
...
with open(model_path+'modellearningCurve.pickle', 'rb') as handle:
...
# In[ ]:
image_dataset = []
for path in glob.glob(DataValPath[0]):
    for img_path in glob.glob(os.path.join(path, "*.jpg")):
        img = cv2.imread(img_path, 1)
        img = cv2.resize(img, (512,512))
        img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
mask_dataset = []
for path in glob.glob(MasksValPath[0]):
    for mask_path in glob.glob(os.path.join(path, "*.jpg")):
        mask = cv2.imread(mask_path, 0)
        mask = cv2.resize(mask, (512,512))
        mask = np.stack((mask, mask), axis=-1)
...
ax[0].imshow(x_val[1][:,:,0] , aspect="auto",cmap='gray')
ax[1].imshow(y_val[1][:,:,0] , aspect="auto",cmap='gray')
ax[2].imshow(y_val[1][:,:,1] , aspect="auto",cmap='gray')
...
file_path = model_path + 'checkpoint'
SavePathVal = save_path + '/patientAnalysisVal/'
